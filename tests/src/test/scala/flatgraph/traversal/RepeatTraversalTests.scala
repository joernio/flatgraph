...
class RepeatTraversalTests extends AnyWordSpec with FlatlineGraphFixture {
  ...
  "typical case for both generic graph steps" in {
    centerTrav.repeat(_.out)(using _.maxDepth(2)).toSet shouldBe Set(l2, r2)
  }

  "repeat given traversal up to depth 2" should {
    "return only the final elements" in {
      val expectedResults = Set(l2, r2)
      centerTrav.repeat(_.out)(using _.maxDepth(2)).toSet shouldBe expectedResults
      centerTrav.repeat(_.out)(using _.maxDepth(2).breadthFirstSearch).toSet shouldBe expectedResults
    }

    "return only the final elements - if any" in {
      val expectedResults = Set(r4) // there is no L4
      centerTrav.repeat(_.out)(using _.maxDepth(4)).toSet shouldBe expectedResults
      centerTrav.repeat(_.out)(using _.maxDepth(4).breadthFirstSearch).toSet shouldBe expectedResults
    }

    "return everything along the way also, if used in combination with emit" in {
      val expectedResults = Set(center, l1, l2, r1, r2)
      centerTrav.repeat(_.out)(using _.maxDepth(2).emit).toSet shouldBe expectedResults
      centerTrav.repeat(_.out)(using _.maxDepth(2).emit.breadthFirstSearch).toSet shouldBe expectedResults
    }
  }

  "emit everything along the way if so configured" in {
    val expectedResults = Set(l3, l2, l1, center, r1, r2, r3, r4, r5)
    centerTrav.repeat(_.out)(using _.emit).toSet shouldBe expectedResults
    centerTrav.repeat(_.out)(using _.emit.breadthFirstSearch).toSet shouldBe expectedResults
  }

  "emit everything along the way but the first element (starting point)" in {
    val expectedResults = Set(l3, l2, l1, r1, r2, r3, r4, r5)
    centerTrav.repeat(_.out)(using _.emitAllButFirst).toSet shouldBe expectedResults
    centerTrav.repeat(_.out)(using _.emitAllButFirst.breadthFirstSearch).toSet shouldBe expectedResults
  }

  "emit nodes along the way that meet given condition" in {
    centerTrav
      .repeat(_.out)(using _.emit(_.where(_.property(StringMandatory).filter(_.startsWith("L")))))
      .property(StringMandatory)
      .toSet shouldBe Set("L1", "L2", "L3")

    // with domain specific language
    centerTrav
      .repeat(_.connectedTo)(using _.emit(_.stringMandatory("L.*")))
      .stringMandatory
      .toSet shouldBe Set("L1", "L2", "L3")
    ...
    centerTrav
      .repeat(_.connectedTo)(using _.maxDepth(2).emit(_.stringMandatory("L.*")))
      .stringMandatory
      .toSet shouldBe Set("L1", "L2", "R2")
  }

  "going through multiple steps in repeat traversal" in {
    r1.start.repeat(_.out.out)(using _.emit).l shouldBe Seq(r1, r3, r5)
    r1.start.enablePathTracking.repeat(_.out.out)(using _.emit).path.l shouldBe Seq(Seq(r1), Seq(r1, r2, r3), Seq(r1, r2, r3, r4, r5))
  }
  ...
  "support arbitrary `until` condition" should {
    "work without emit" in {
      ...
      centerTrav
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_.matches(".*2"))))
        .property(StringMandatory)
        .toSet shouldBe expectedResults

      centerTrav
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_.matches(".*2"))).breadthFirstSearch)
        .property(StringMandatory)
        .toSet shouldBe expectedResults
    }
    ...
    "work in combination with emit" in {
      val expectedResults = Set("Center", "L1", "L2", "R1", "R2")
      centerTrav
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_.matches(".*2"))).emit)
        .property(StringMandatory)
        .toSet shouldBe expectedResults
      centerTrav
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_.matches(".*2"))).emit.breadthFirstSearch)
        .property(StringMandatory)
        .toSet shouldBe expectedResults
    }
    ...
    "result in 'repeat/until' behaviour, i.e. `until` condition is only evaluated after one iteration" in {
      ...
      centerTrav.repeat(_.out)(using _.until(_.hasLabel(NodeA.Label))).property(StringMandatory).toSet shouldBe expectedResults
      centerTrav
        .repeat(_.out)(using _.until(_.hasLabel(NodeA.Label)).breadthFirstSearch)
        .property(StringMandatory)
        .toSet shouldBe expectedResults
    }
    ...
    "be combinable with `.maxDepth`" in {
      centerTrav
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_ == "R2")).maxDepth(3))
        .property(StringMandatory)
  ...
  "until and maxDepth" should {
    "work in combination" in {
      centerTrav.repeat(_.out)(using _.until(_.property(StringMandatory).filter(_ == "R2")).maxDepth(2)).toSet shouldBe Set(l2, r2)
      centerTrav
        .repeat(_.out)(using _.breadthFirstSearch.until(_.property(StringMandatory).filter(_ == "R2")).maxDepth(2))
        .toSet shouldBe Set(l2, r2)
    }

    "work in combination with path" in {
      centerTrav.enablePathTracking
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_ == "R2")).maxDepth(2))
        .path
        .filter(_.last == r2)
  ...
  "support repeat/while behaviour" should {
    "base case: given `whilst` condition is also evaluated for first iteration" in {
      centerTrav.repeat(_.out)(using _.whilst(_.property(StringMandatory).filter(_ == "does not exist"))).toSet shouldBe Set(center)
    }

    "walk one iteration" in {
      centerTrav.repeat(_.out)(using _.whilst(_.property(StringMandatory).filter(_ == "Center"))).toSet shouldBe Set(l1, r1)
    }
    ...
    "emitting nodes along the way" in {
      centerTrav.repeat(_.out)(using _.emit.whilst(_.property(StringMandatory).filter(_ == "Center"))).toSet shouldBe Set(center, l1, r1)
      centerTrav.repeat(_.out)(using _.emitAllButFirst.whilst(_.property(StringMandatory).filter(_ == "Center"))).toSet shouldBe Set(l1, r1)
    }

    "with path tracking enabled" in {
      centerTrav.enablePathTracking
        .repeat(_.out)(using _.whilst(_.property(StringMandatory).filter(_ == "Center")))
        .path
    ...
    "be combinable with `.maxDepth`" in {
      centerTrav
        .repeat(_.out)(using _.whilst(_.property(StringMandatory).filter(_ != "R2")).maxDepth(3))
        .property(StringMandatory)
        .toSet shouldBe Set("L3", "R2")
    }
  }
  ...
  ".dedup should apply to all repeat iterations - e.g. to avoid cycles" when {
    ...
    "path tracking is not enabled" in {
      centerTrav.repeat(_.both)(using _.maxDepth(2).dedup).toSet shouldBe Set(l2, r2)
      centerTrav.repeat(_.both)(using _.maxDepth(3).dedup).toSet shouldBe Set(l3, r3)
      centerTrav.repeat(_.both)(using _.maxDepth(4).dedup).toSet shouldBe Set(r4)

      // for reference, without dedup (order is irrelevant, only using .l to show duplicate `center`)
      centerTrav.repeat(_.both)(using _.maxDepth(2)).l shouldBe Seq(l2, center, r2, center)
    }

    "path tracking is enabled" in {
      centerTrav.enablePathTracking.repeat(_.both)(using _.maxDepth(2).dedup).path.toSet shouldBe Set(
        Seq(center, l1, l2),
        Seq(center, r1, r2)
      )

      // for reference, without dedup:
      centerTrav.enablePathTracking.repeat(_.both)(using _.maxDepth(2)).path.toSet shouldBe Set(
        Seq(center, l1, l2),
    ...
    "used with emit" in {
      ...
      centerTrav.repeat(_.both)(using _.maxDepth(2).emit.dedup).l shouldBe Seq(center, l1, l2, r1, r2)
    }

    "used with emit and path" in {
      centerTrav.enablePathTracking.repeat(_.both)(using _.maxDepth(2).emit.dedup).path.toSet shouldBe Set(
        Seq(center),
        Seq(center, l1),
        Seq(center, l1, l2),
        Seq(center, r1),
  ...
  "is lazy" in {
    ...
    val traversalNotYetExecuted = {
      ...
      centerTrav.repeat(_.out.sideEffect(traversedNodes.addOne))(using _.breadthFirstSearch)
    }
  ...
  "hasNext check doesn't change contents of traversal" when {
    "path tracking is not enabled" in {
      val trav = centerTrav.repeat(_.out)(using _.maxDepth(2))
      trav.hasNext shouldBe true
      trav.toSet shouldBe Set(l2, r2)
    }

    "path tracking is enabled" in {
      val trav1 = centerTrav.enablePathTracking.repeat(_.out)(using _.maxDepth(2))
      val trav2 = centerTrav.enablePathTracking.repeat(_.out)(using _.maxDepth(2)).path
  ...
  "traverses all nodes to outer limits exactly once, emitting and returning nothing, by default" in {
    ...
    test(centerTrav.repeat(_.sideEffect(traversedNodes.addOne).out)(using _.breadthFirstSearch).l)
  ...
  "uses BFS (breadth first search) if configured" in {
    val traversedNodes = mutable.ListBuffer.empty[GNode]
    centerTrav.repeat(_.sideEffect(traversedNodes.addOne).out)(using _.breadthFirstSearch).iterate()
  ...
  "hasNext is idempotent: DFS" in {
    val traversedNodes = mutable.ListBuffer.empty[GNode]
    val traversal      = centerTrav.repeat(_.sideEffect(traversedNodes.addOne).out)(using _.maxDepth(3))
  ...
  "hasNext is idempotent: BFS" in {
    val traversedNodes = mutable.ListBuffer.empty[GNode]
    val traversal      = centerTrav.repeat(_.sideEffect(traversedNodes.addOne).out)(using _.maxDepth(2).breadthFirstSearch)
  ...
  "supports large amount of iterations" in {
    ...
    genericDomain.nodeA.stringMandatory("a").repeat(_.out)(using _.maxDepth(repeatCount)).property(StringMandatory).l shouldBe List("b")
    genericDomain.nodeA
      .stringMandatory("a")
      .repeat(_.out)(using _.maxDepth(repeatCount).breadthFirstSearch)
      .property(StringMandatory)
      .l shouldBe List("b")
  ...
  "support .path step" when {
    "using `maxDepth` modulator" in {
      centerTrav.enablePathTracking.repeat(_.out)(using _.maxDepth(2)).path.toSet shouldBe Set(Seq(center, l1, l2), Seq(center, r1, r2))
    }

    "using `emit` modulator" in {
      centerTrav.enablePathTracking.repeat(_.out)(using _.emit).path.toSet shouldBe Set(
        Seq(center),
        Seq(center, l1),
        Seq(center, l1, l2),
        Seq(center, l1, l2, l3),
    ...
    "using `until` modulator" in {
      centerTrav.enablePathTracking
        .repeat(_.out)(using _.until(_.property(StringMandatory).filter(_.endsWith("2"))))
        .path
    ...
    "using breadth first search" in {
      centerTrav.enablePathTracking
        .repeat(_.out)(using _.breadthFirstSearch.maxDepth(2))
        .path
        .toSet shouldBe Set(Seq(center, l1, l2), Seq(center, r1, r2))
    }

    "doing multiple steps: should track every single step along the way" in {
      centerTrav.enablePathTracking.repeat(_.out.out)(using _.maxDepth(1)).path.toSet shouldBe Set(Seq(center, l1, l2), Seq(center, r1, r2))

      r1.start.enablePathTracking
        .repeat(_.out.out.out)(using _.maxDepth(1))
        .path
        .toSet shouldBe Set(Seq(r1, r2, r3, r4))

      r1.start.enablePathTracking.repeat(_.out.out)(using _.maxDepth(2)).l shouldBe Seq(r5)
      r1.start.enablePathTracking.repeat(_.out.out)(using _.maxDepth(2)).path.next() shouldBe List(r1, r2, r3, r4, r5)
    }

    "should not forget steps preceding the repeat" in {
      centerTrav.enablePathTracking.out
        .repeat(_.out.out)(using _.maxDepth(1))
        .path
  ...
  "type resolution in hierarchical domain schema" in {
    ...
    nodeX.repeat(_.connectedTo)(using _.emit).name.l shouldBe List("X1", "Y2", "X3", "Y4")
  }
