...
object Foo {
  def main(args: Array[String]): Unit = {
    //val schema2 = o.shiftleft.codepropertygraph.schema.CpgSchema.instance
    val schema = io.shiftleft.codepropertygraph.schema.CpgExtSchema.instance
    ...
    if (actualPropertiesSet.size != actualPropertiesSet.map {
      _.name
    }.size) {
      println(actualPropertiesSet.toList.sortBy {
        _.name
      })
      println(actualPropertiesSet.map {
        _.name
      }.toList.sorted)
      ???
    }
    ...
    val edgeAccess = edgeTypes.map{et =>
      s"""def _${Helpers.camelCase(et.name)}Out: IndexedSeq[StoredNode] = odb2.Accessors.getNeighborsOut(this.graph, this.nodeKind, this.seq, ${edgeIdByType(et)}).asInstanceOf[IndexedSeq[StoredNode]]
         |def _${Helpers.camelCase(et.name)}In: IndexedSeq[StoredNode] = odb2.Accessors.getNeighborsIn(this.graph, this.nodeKind, this.seq, ${edgeIdByType(et)}).asInstanceOf[IndexedSeq[StoredNode]]
         |""".stripMargin
    }.mkString("\n")
    val basefile =
    ...
    val basetypefile = schema.nodeBaseTypes
      .map { baseType =>
        ...
        val accessors = newProperties.map { p =>
        ...
        s"""trait ${baseType.className}Base extends ${mixinsBase.mkString(" with ")}
           ...
           | // inherited properties: ${oldProperties.mkString(", ")}
          ...
          oldExtendz.map {
           ...
           | // implementing nodes: ${
          nodeTypes.filter { n => n.extendzRecursively.contains(baseType) }.map {
            _.name
          }.mkString(", ")
        }
           |trait ${baseType.className} extends ${
          mixinsStored
            .mkString(" with ")} {
            |${accessors}
            |}
      ...
      .map { case (edgeType, idx) =>
        ...
        val acceesor = if (edgeType.properties.length == 1) {
          ...
          p.cardinality match {
            ...
            case Cardinality.ZeroOrOne =>
              s"""{
                 |  def ${Helpers.camelCase(p.name)}: Option[${
                unpackTypeUnboxed(
                  p.valueType,
                  true
                )
      ...
      s"""package ${basePackage}
        ...
        nodeTypes
          ...
          .map { case (node, contained) =>
            s"    else if(propertyKind == ${
              actualProperties.length + containedIndexByName(
                contained.localName
              )
    ...
    val propertyBaseConversions = (schema.nodeBaseTypes ++ schema.nodeTypes)
      .flatMap { typ => typ.properties.toList.map { p =>
      ...
      .sorted
      .mkString("\n")
    val propCon = schema.nodeTypes.flatMap{typ => typ.containedNodes.map{ contained =>
      val cc = s"Access_contained_${contained.localName}_${typ.name}"
      s"implicit def ${typ.className}_accessContained_${contained.localName}(node: nodes.${typ.className}): $cc = new $cc(node)"
    }}.mkString("\n")
