 ...
 * deviated from the spec, but given that other tools don't support it and the complications re re-importing, we
 * also dropped support for lists. Now, lists are dropped and we print a warning.
 *
